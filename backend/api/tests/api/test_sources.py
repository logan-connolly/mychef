...
class TestSource:
    ...
    def test_get_source(self, client):
        resp = client.get(f"{settings.api_version}/sources/{SOURCE_ID}/")
        assert resp.status_code == 200
        assert resp.json() == SOURCE

    def test_get_sources(self, client):
        resp = client.get(f"{settings.api_version}/sources/")
        assert resp.status_code == 200
    ...
    def test_update_source(self, client):
        ...
        resp = client.put(f"{settings.api_version}/sources/{SOURCE_ID}/", data=payload)
        assert resp.status_code == 200
        assert resp.json() == SOURCE
