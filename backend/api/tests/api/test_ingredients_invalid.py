...
class TestIngredientInvalid:
    ...
    def test_get_ingredient_invalid(self, client):
        resp = client.get(f"{settings.api_version}/ingredients/0/")
        assert resp.status_code == 404

    def test_get_ingredients_invalid(self, client):
        resp = client.get(f"{settings.api_version}/ingredient/")
        assert resp.status_code == 404

    def test_update_ingredient_invalid(self, client):
        resp = client.put(
            f"{settings.api_version}/ingredients/0/", data=json.dumps(dict())
        )
        assert resp.status_code == 404

    def test_remove_ingredient_invalid(self, client):
        resp = client.delete(f"{settings.api_version}/ingredients/0/")
        assert resp.status_code == 404
