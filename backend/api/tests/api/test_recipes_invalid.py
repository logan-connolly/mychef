...
class TestRecipeInvalid:
    ...
    def test_add_recipe_invalid(self, client, recipe, code):
        resp = client.post(
            f"{settings.api_version}/sources/1/recipes/", data=json.dumps(recipe)
        )
        assert resp.status_code == code

    def test_get_recipe_invalid(self, client):
        resp = client.get(f"{settings.api_version}/sources/{SOURCE_ID}/recipes/0/")
        assert resp.status_code == 404

    def test_get_recipes_invalid(self, client):
        resp = client.get(f"{settings.api_version}/sources/0/recipes/")
        assert resp.status_code == 404

    def test_update_recipe_invalid(self, client):
        resp = client.put(
            f"{settings.api_version}/sources/{SOURCE_ID}/recipes/0/",
            data=json.dumps(dict()),
        )
        assert resp.status_code == 404

    def test_remove_recipe_invalid(self, client):
        resp = client.delete(f"{settings.api_version}/sources/{SOURCE_ID}/recipes/0/")
        assert resp.status_code == 404
